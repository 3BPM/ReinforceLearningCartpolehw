...
# 【【【新功能】】】 更新窗口标题
pygame.display.set_caption("LQR自平衡小车 (L:开关LQR, R:重置, 空格:暂停, ↑/↓:速度)")
...
while running:
    ...
    if not is_paused:
        ...
        if is_lqr_active:
            ...
            u_lqr = np.array([0.0]) # 如果关闭，LQR力为0
    ...
    if is_lqr_active:
        lqr_force_text = f"LQR计算力: {(-K @ x)[0]:.2f} N"
    else:
        lqr_force_text = "LQR计算力: 0.00 N"
